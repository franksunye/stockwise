...
本文件定义 知守 AI (ZISO AI) 面向用户的核心领域实体与命名边界，特别是 **“择时、决议、交易”** 的决策闭环逻辑，避免产品、前端、后端、内容运营出现同物多名。

### 1.1 核心决策逻辑：三位一体 (The Decision Trinity)

知守 AI 的核心价值在于将复杂的量化分析压缩为用户可感知的三个维度：

1.  **择时 (Timing / When)**：回答 **“什么时候动？”**。由量化引擎产出的入场窗口、风险缓冲位与观察点位构成，解决时机问题。
2.  **决议 (Council / What)**：回答 **“听谁的？”**。由 AI 智囊团（AICouncil）对多路 Producer 结果进行审计与综合，解决共识与偏见问题。
3.  **交易 (Trade / How)**：回答 **“怎么落地？”**。针对真实持仓的纪律管理卡（交易决策卡），解决具体执行与利润保护问题。
...
| 择时 | Timing Brief | Timing | 围绕单只股票的入场/观察点位与动态预判面板（原称策略内参）。 |
| 投研决议 | Investment Decision | Decision | 多分析引擎（AICouncil）对同一标的的综合结论。 |
| 交易决策 | Trade Execution | Trade | 针对具体持仓的纪律管理、执行建议与账本记录（原称管理）。 |
...
- 历史旧名 `策略内参 / Tactical Brief` 统一收敛为 `择时 / Timing Brief`。
- 历史旧名 `管理 / Management` 统一收敛为 `交易 / Trade Execution`。
- 对外内容、前台文案、帮助中心与增长素材应优先使用 `择时 / 决议 / 交易` 三位一体口径，避免新旧混用。
...
| 量化工程师 | Quant Engineer | 负责数据处理、指标计算、规则实现、参数管理与量化模型落地的工程岗位 | 量化工程师 |
| 研究负责人 | Research Lead | 负责量化 + AI 研究框架、方法演进与术语统一的统筹岗位 | 研究负责人 |
