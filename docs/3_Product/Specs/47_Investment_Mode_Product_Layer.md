...
随着交易管理进入正式研究阶段，产品层还必须接受一个新的设计约束：

- 后台研究允许使用多把行业尺子同时比较模型
- 前台产品不应把这套评分板直接展示给普通用户
...
StockWise 在中国市场语境下，必须明确区分 **价格颜色** 与 **决策颜色**，避免直接照搬海外产品中“绿=好、红=坏”的默认习惯。

1. **价格层遵守中国用户直觉**
   - 上涨 / 浮盈 / 红盘：使用红色
   - 下跌 / 浮亏 / 绿盘：使用绿色
2. **决策层不使用红绿二元对立作为唯一语义**
   - `TriggeredLong` / `TrendHolding`：可使用暖红系或正向强调色
   - `Watch` / `BreakoutPending`：使用蓝色
   - `ProfitProtection`：使用琥珀色
   - `RiskOff` / `FailureRisk`：使用深橙、棕红或警示色
3. **同一套颜色母语必须贯穿信号与交易管理**
   - 不允许信号层、交易管理层、前端表现层各自发明不同颜色系统
   - 同一颜色应表达相近的决策语义，只允许在不同模块中做深浅变化

结论：

**价格颜色服务“市场事实”，决策颜色服务“系统判断”。两者必须分层，而不能混用。**
...
补充到交易管理产品化语境里，这句话可以进一步收口为：

- 用户不需要学习 `Sharpe`、`DSR`、`PBO`
- 用户需要知道：
  - 现在是什么状态
  - 应该怎么做
  - 错了在哪里退
...
**产品的工作不是把研究指标搬到前台，而是把研究指标压缩成用户可执行的纪律输出。**
...
### 2.3.a 新增边界：模式与交易管理引擎的关系

为避免后续产品和工程继续混淆，这里明确：

1. 当前 `Investment Mode` 负责的是：
   - 把同一套市场判断转译成不同风险风格
   - 让用户选择“更稳 / 更平衡 / 更进取”的决策偏好
2. 当前 `Investment Mode` 还不负责完整解决：
   - 已有仓位时何时部分止盈
   - 何时移动纪律位
   - 何时允许加仓
   - 何时认定失败突破并退出
3. 这些问题属于未来的 **交易管理引擎（Trade Management Engine）**
   - 它需要基于：
     - 信号状态
     - 持仓状态
     - 管理策略
...
**Investment Mode 是交易管理产品层的第一阶段，不应被错误包装成完整交易管理系统。**

### 2.3.b 面向用户与面向系统的双层结构

为保持产品简洁与内部严谨，交易管理产品层采用固定双层结构：
...
### 2.3.c 交易管理产品化的最终压缩形式

为了避免未来产品层不断膨胀，这里补一个收口原则：

无论后台研究使用多少指标、多少 lane、多少验证方法，前台最终都应尽量压缩成三类输出：
...
- 已经落袋了多少
- 当前剩余仓位承担的风险有多大
- 同一条建议是在“利润已厚”还是“还没锁利润”的前提下给出的

#### B. 建议 vs 实际执行偏差层

如果未来要让交易管理真正形成长期产品价值，就不能只展示“建议是什么”，还要能回答：

- 用户有没有执行
- 是提前、延后、还是反向执行
- 偏差来自系统判断问题，还是执行纪律问题

这意味着单持仓详情页最终会从“信息并列”走向“偏差对照”。

#### 当前产品原则

这两条都属于下一阶段正式待办，但当前先不抢工程优先级。
...
**已实现盈亏层与偏差视图层，已进入产品后续规划，但当前不作为阻塞内部试用版的前置条件。**

### 2.3.f 从好讨论到真功能的落地方式

交易管理后续所有新想法，都不应直接以“多加一个提示”进入产品，而应沿固定收口路径落地：
...
- 不允许先讨论产品文案，再回头拼后台脏字段。
- 不允许先造前台样式，再回头补生命周期对象。
- 任何“好讨论”如果不能收成稳定对象、稳定逻辑、稳定表现三层之一，默认不进入近期功能面。

结论：

**交易管理的产品化，不是不断增加建议句子，而是把讨论收成“对象 -> 逻辑 -> 表现”的稳定链路。**

### 2.3.g 建议卡 V2：从静态建议到剩余仓位建议

当前内部试用版建议卡，已经从“单笔静态持仓建议”升级为“带最近执行上下文的剩余仓位建议卡”。
...
**建议卡 V2 的产品价值，不再只是“告诉用户明天怎么做”，而是告诉用户“在已经发生过真实操作之后，剩余仓位现在该怎么管”。**
...
如果后续引入“执行卡”“部分止盈建议”“持仓管理计划”等能力，必须遵守以下原则：

1. 不把研究层临时结论直接硬编码成前台规则
2. 不把单个案例经验包装成“普适最优管理法”
3. 必须先完成交易管理研究层的比较与校准，再进入正式产品表达
4. 用户侧仍优先看到简单动作语言，而不是复杂管理参数
...
### 7.3.1 Dashboard 时光机产品语义

该规则属于产品语义，不属于单一端的实现自由裁量。`/dashboard` 在历史模式下的横滑与纵滑，必须遵守统一的时间上下文规则。

交互原则：
- 横滑是“切股票”，纵滑是“切当前股票的时间层”。
- 当用户已经进入某只股票的历史卡片时，横滑不应粗暴打断其时间上下文。
- 历史模式下，系统优先保持“同一历史日期”的视角，而不是强制回到今天。

切股优先级：
- 用户在 A 股票的历史卡片横滑到 B 股票时，优先落到 B 股票的“同一日期”历史卡片。
- 若 B 股票不存在该日期，则落到 B 股票“最近的历史卡片”。
- 最近历史卡片优先选择更早的最近交易日，避免跳到当前用户尚未浏览的未来日期。
- 若 B 股票无任何可用历史卡片，则回到 B 股票的今天主卡。
- 若用户原本就在 A 股票的今天主卡，则横滑到 B 股票后仍默认进入 B 股票的今天主卡。

语义优先级：
- `同日期历史卡 > 最近历史卡 > 今天主卡`
- 禁止将“第 N 张卡片”本身视为产品语义；真正需要保持的是“时间锚点”。

“回到今天”按钮规则：
- “回到今天”只负责当前股票在纵向时间层内回到今天主卡。
- 该按钮的显示与隐藏，只能由“当前股票当前纵向层级”决定，不能沿用前一只股票的状态。
- 用户在历史卡点击“回到今天”后，只回到当前股票的主卡，不得影响其他股票的历史定位。

边界场景：
- 历史列表日期缺失、遇到休市日或节假日时，允许回退到最近可用交易日，但不得制造“未来日期穿越”感。
- 不同股票历史卡数量不一致时，必须按日期语义回退，不能简单按序号硬对齐。
- 历史卡内容保持生成当日结论，不允许因当前模式或后续补算造成前台“回写历史”的感知。

### 7.3.2 前端实现约束与验收

实现约束：
- Android WebView、iPhone Safari、PWA 必须遵守同一交互语义，不允许因滚动实现差异改变产品行为。
- 前端实现不得把横向页码、纵向层级、按钮状态彼此串用；三者都必须绑定当前股票上下文。
- 横向切股时，前端应优先以“日期锚点”而非“第 N 张卡片序号”驱动目标定位。
- 若目标股票不存在当前日期，前端必须执行最近日期回退，再退到今天主卡，不得出现空白落点。
- 若平台滚动精度存在差异，允许在实现层做吸附与定位补偿，但不得改变以上用户可感知语义。

推荐状态表达：
- 当前股票：`activeStock`
- 当前时间层类型：`today | history`
- 当前时间锚点：`activeLayerDate | null`

验收标准：
- 用户在 A 股票某历史日期横滑到 B 股票时，B 优先落在同日期历史卡。
- B 无同日期时，落在最近可用历史卡；B 无历史时，落在今天主卡。
- “回到今天”按钮在当前股票进入历史层后稳定出现，返回今天后稳定消失。
- 连续横滑到后段股票时，Android 与 iPhone 均不得出现按钮状态串页、历史层错位或落点错误。
- 横滑完成前允许存在过渡动画，但横滑完成后的最终落点必须符合 `同日期历史卡 > 最近历史卡 > 今天主卡`。

交互时序示例：
- 示例 A：用户在 A 股票的 `03/13` 历史卡，向左滑到 B 股票；若 B 存在 `03/13` 历史卡，则直接落在 B 的 `03/13` 历史卡，“回到今天”按钮保持可见。
- 示例 B：用户在 A 股票的 `03/13` 历史卡，向左滑到 B 股票；若 B 不存在 `03/13`，但存在 `03/12` 与 `03/10`，则默认落在 `03/12`，不回到今天。
- 示例 C：用户在 A 股票的 `03/13` 历史卡，向左滑到 B 股票；若 B 没有任何历史卡，则落在 B 的今天主卡，“回到今天”按钮隐藏。
- 示例 D：用户在 A 股票今天主卡，向左滑到 B 股票；无论 B 历史数据多完整，默认仍落在 B 的今天主卡。
- 示例 E：用户在 B 股票历史卡点击“回到今天”后，只回到 B 的今天主卡；随后再横滑到 C 股票时，应按 C 自身的时间层规则重新判断，不得延用 B 的按钮状态或历史落点。
