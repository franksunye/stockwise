...
## 1. 核心目标 (The Goal)
在不破坏现有高质量 UI 体验的前提下，通过**微妙的兵法级文案修订 (Semantic Text Changes)** 与 **团队服务模式的转换 (Team Service Narrative)**，将旧的“单个AI孤立喊单”体系，升级为“小组协作服务（顾深+程矩 / 林序+程矩）”的心智模型。后台由沈策负责量化工程底座，但前台默认不把量化工程角色做成强露出。用极简的文字进化，匹配底层双轨系统的威力。
...
- **表现**：
    - **体现协同**：在策略内参和投研决议中，文案及角色名需侧面体现这是“程矩（规则分析）提供规则侧判断，顾深/林序（分析师）提供解读与推演”的团队包裹服务。
...
| **`AICouncil.tsx`**           | 下方成员卡片列表       | 独立工作的成员报告展示                           | 展示为“小组服务”协同推演（如顾深+程矩），必要时可在次级说明中提及由沈策提供量化底座。 | 保持现有高亮逻辑（`isPrimary`）。            | 对旧数据完全无痛。        |
| **`TacticalBriefDrawer.tsx`** | 战术抽屉 (Drawer)      | 一人的长篇独立分析                               | 文案结构与署名上体现为“小组服务结果”，例如程矩给规则侧判断、林序解释逻辑。 | 原有 markdown 渲染层保持不变。               | -                         |
