...
12. `Dashboard Data Refresh Contract` 第一轮已完成：
...
13. 新用户首次进入 Dashboard 的修复链路已经落地。
...
1. 非首帧关键数据面的 SWR 迁移尚未系统推进，但 `Brief`、`StockProfile`、`UserCenterDrawer`、`TacticalBriefDrawer` 与其邻接 content surface 五个面已完成第一轮低风险收口。
