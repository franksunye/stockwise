...
**HK 交易管理的最终目标，是在统一状态机框架下，构建一个能够识别港股不同失败/修复路径、并输出差异化执行纪律的路径感知型持仓管理系统。**

### 12.1A 终局目标

在系统定义之上，`HK` 的业务终局目标应明确为：

1. 在正式 `HK` 样本中长期追求收益与风险双改善
2. 以“效果最优”为方向持续优化 `HK` 持仓管理表现
3. 长期以“行业领先的港股交易管理引擎”为目标，而不是停留在“合理可用”

### 12.2 终局的正式内涵

`HK` 终局不定义为“另一套独立模型”，而定义为：

1. 继续沿用统一六状态契约
2. 在 `HK` 上形成更强的路径识别能力
3. 在路径识别之上形成稳定、克制、可解释的 `policy family`
4. 在正式 `HK` 样本中，长期表现为收益与风险双改善的管理引擎
...
2. `路径分化`
   - `HK` 的分化重点不在 state 名称，而在：
   - `low-side / false-recovery / failure-loop` 路径识别
   - 不同路径下的执行分支选择

3. `执行分化`
   - `HK` 最终要形成稳定的执行家族，而不是只靠统一阈值切换：
   - 保留弹性
   - 轻度止盈
   - 轻度风险压缩
   - 中度风险压缩
...
4. `产品表达不分裂`
   ...
   - `动作`
   - `纪律`
   - 用户不需要理解内部 subtype，但应感受到“这套建议更懂港股”
...
2. 不做 `HK` 专属前台产品壳层
3. 不追求无限扩张 policy 数量
4. 不为了短期回测增益而牺牲整体可解释性

### 12.5 三阶段演进路径

`Phase A`
- 统一状态机 + market-aware routing
- 目标：证明 `HK` 不应继续使用 `CN` 默认路由

`Phase B`
- subtype-aware policy family
- 目标：让 `HK` 从“阈值分化”升级为“路径分化”

`Phase C`
- 路径感知型执行系统
- 目标：在 `HK` 正式样本中长期稳定表现为 `dual improver`

### 12.6 当前所处位置

当前 `HK` 已进入：

**`Phase B: subtype-aware policy family`**

这意味着：

1. 已脱离纯统一阈值阶段
2. 已开始按 subtype 区分 `HK` 低侧路径
3. 但仍未到完整的“路径感知型执行系统”终局
4. 距离“效果最优 / 行业领先”的终局目标仍有明显研究与实盘验证空间
