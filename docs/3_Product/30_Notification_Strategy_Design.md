...
#### 2. 开盘早报 (morning_call)
> 触发逻辑：于开盘前 15~30 分钟固定触发。不采用宏观大盘情绪，而是直接聚合用户**个人自选股池 (Watchlist)** 当日的 AI 预测核心语义（TriggeredLong / RiskOff）。系统采用基于“绝对触发量”的稀疏适配算法（Sparsity Adjusted）为每个用户输出千人千面的 `{sentiment_tag}` 与 `{stock_names}` 组合：
