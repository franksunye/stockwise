...
1. `daily_prices` 覆盖标的数：`79 -> 301`
2. `daily_prices` 总行数：`37907 -> 154118`
3. 最近交易日覆盖标的数：`73 -> 298`
4. 当前板块分布：
   - `6` 开头：`133`
   - `0` 开头：`88`
...
结论：本地样本已从“原型验证”提升到“中等规模研究样本”，足以支持下一轮 `tradeability_v2` 稳定性评估。
...
结论：本地观测面板已不再依赖单日 sidecar 结果，而具备连续历史状态轨迹。
...
经 `backend/scripts/run_tradeability_weekly_calibration.py` 在 `301` 个 CN 标的样本上按单参数顺序调优后，当前本地研究基线参数更新为：
...
1. `tradeability_v1`
   - `Triggered coverage = 1.92%`
   - `RiskOff = 52.05%`
   - `Max DD = 91.46%`
   - `T+3 win = 47.78%`
2. `tradeability_v2`
   - `Triggered coverage = 5.11%`
   - `RiskOff = 53.67%`
   - `Max DD = 83.79%`
   - `T+3 win = 50.09%`
...
1. 新 `v2` 已在本地 `301` 样本下跨过 `TriggeredLong coverage > 5%` 目标线。
