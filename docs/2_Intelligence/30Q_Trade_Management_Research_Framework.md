...
截至 `2026-03-29`，StockWise 已完成第一版最小组合级业绩基线。
...
1. `equal_notional_per_case_with_idle_cash_before_entry`
   - 每个 case 分配同等初始资金
   - 入场前资金为空闲现金
   - 平仓后资金不在同一轮 case pool 中再利用
2. `routed` 与 `buy-and-hold baseline` 使用同一批 case
3. benchmark 使用市场匹配 ETF：
   - `CN -> 510300 (沪深300ETF)`
   - `HK -> 02800 (恒生指数ETF)`
...
4. 组合最大回撤：约 `-0.53%`
5. 路由后最大回撤：约 `-0.30%`
6. 相对市场匹配 benchmark 组合超额：约 `+0.81%`

说明：

1. 当前窗口仍然较短，因此 `annualized_return / sharpe_like` 只作为研究参考，不作为对外主表述
2. 当前最可信、最适合对内对外统一使用的表述仍是：
   - `total_return`
   - `max_drawdown`
...
**当前交易管理模型已经具备“组合级业绩基线”，不再只是 case 均值或单票演示。**

### 5F. 当前 6M 组合级业绩基线（已完成）

截至 `2026-03-29`，StockWise 已进一步完成第一版 `6M` 组合级业绩基线。

这次基线不是重新定义口径，而是在既有正式研究池和正式路由规则之上，把历史窗口从近 `3M` 往前扩展到当前本地可支撑的近 `6M` 区间。

前提说明：

- `CN` benchmark 继续使用 `510300`（沪深300ETF）
- `HK` benchmark 继续使用 `02800`（恒生指数ETF）
- 由于本地 `rule-engine primary` 与 `tradeability_v2` 的正式全栈覆盖此前从 `2025-12-29` 起步，本次先补齐了 `2025-10-16..2025-12-28` 的历史覆盖，再生成新的 `6M` case pool
- 当前 `6M` 仍属于“研究正式基线”，不是最终资金复用、仓位约束、交易成本 fully realistic 的终局口径

#### 5F.1 CN500（6M）

- baseline 组合总收益：`+10.73%`
- routed 组合总收益：`+9.40%`
- 相对 baseline 改善：`-1.33%`
- baseline 最大回撤：`-8.40%`
- routed 最大回撤：`-7.52%`
...
- 在 `CN500` 的 `6M` 窗口里，当前交易管理路由**没有提升原始总收益**
- 但它**降低了组合回撤**，并且提高了 `Sharpe-like`
- 这说明当前 `Trade Management v1` 在 `CN` 长窗口下，更像一个**风险优化器**，而不是收益放大器
...
- 在 `HK180` 的 `6M` 窗口里，当前交易管理路由同时改善了：
  - 总收益
  - 最大回撤
  - 相对 benchmark 超额
- 这说明当前 `Trade Management v1` 在 `HK` 上的长窗口适配性更强
...
- baseline 最大回撤：`-6.46%`
- routed 最大回撤：`-5.71%`
- 相对市场匹配 benchmark 组合超额：
  - baseline：`+10.37%`
  - routed：`+9.77%`

解释：

- 在合并组合视角下，当前交易管理路由仍然体现为：
  - **收益略低于简单持有**
  - **但回撤更低，风险收益比更好**
- 因此当前 `6M` 基线给出的正式结论不是“当前交易管理一定更赚钱”，而是：
  - 在 `CN` 更偏风险优化
  - 在 `HK` 已表现出更完整的收益与风险双改善

#### 5F.4 当前阶段正式判断

当前 `6M` 基线允许我们更诚实也更完整地对外表达：

1. `Trade Management v1` 已经具备正式的长窗口组合级业绩基线，不再停留在短窗口或 case 均值层面。
2. 这套模型当前最稳定的优势是：
   - 控制回撤
   - 控制利润回吐
   - 在弱市场 benchmark 下维持更好的相对表现
3. 这套模型当前还不能统一宣称：
   - 在所有市场、所有窗口下都比简单持有更高收益
4. 因此当前最准确的产品表达应是：
   - **这是一个已证明能改善组合风险表现、并在部分市场实现收益改进的交易管理模型**
   - 不是一个已经完成所有市场统一收益最优证明的终局模型

### 5G. 当前 12M 组合级业绩基线（已完成）

截至 `2026-03-29`，StockWise 已进一步完成第一版 `12M` 组合级业绩基线。

这次基线是在 `6M` 口径之上继续往前扩展，而不是重新定义方法：

- 先补齐 `2025-03-27..2025-10-15/22` 的 `daily_prices`
- 再补齐同窗口的 `rule-engine primary`
- 再补齐同窗口的 `tradeability_v2`
- 最后生成：
  - `CN full-stack 500-case`
  - `HK full-stack 180-case`
  - `CN+HK full-stack 680-case`

当前 `12M` 仍然属于“正式研究基线”，不是 fully realistic 的终局组合口径；但它已经能正式回答用户最关心的问题：

- 长窗口下，这套交易管理是否真的改善了结果
- 它是收益优化器，还是风险优化器
- 相对市场匹配 benchmark，当前到底站在什么位置
...
- baseline 最大回撤：`-6.91%`
- routed 最大回撤：`-6.02%`
- baseline Sharpe-like：`1.15`
- routed Sharpe-like：`1.13`
- 相对 `沪深300ETF` 超额：
  - baseline：`-0.11%`
  - routed：`-1.09%`
...
- 在 `CN500` 的 `12M` 窗口里，当前交易管理路由继续降低了回撤与波动
- 但它没有提升原始总收益
- 同时，相对 `沪深300ETF` 的长窗口表现也没有跑赢

因此，在 `CN` 的 `12M` 长窗口里，当前 `Trade Management v1` 的正式定位应当是：
...
- baseline 最大回撤：`-1.11%`
- routed 最大回撤：`-0.88%`
- baseline Sharpe-like：`0.31`
- routed Sharpe-like：`0.73`
- 相对 `恒生指数ETF` 超额：
  - baseline：`-2.42%`
  - routed：`-2.11%`
...
- 在 `HK180` 的 `12M` 窗口里，当前交易管理路由仍然实现了：
  - 收益提升
  - 回撤下降
  - 风险收益比明显改善
- 但长窗口下，`HK` 研究池整体仍然没有跑赢 `恒生指数ETF`

因此，在 `HK` 的 `12M` 长窗口里，当前 `Trade Management v1` 的正式定位是：

- **在研究池内继续成立，但还没有强到足以稳定跑赢市场基准**
...
- baseline 最大回撤：`-5.23%`
- routed 最大回撤：`-4.53%`
- baseline Sharpe-like：`1.13`
- routed Sharpe-like：`1.14`
- 相对市场匹配 benchmark 组合超额：
  - baseline：`-0.72%`
  - routed：`-1.36%`
...
- 在合并组合层面，当前 `Trade Management v1` 依旧主要贡献在：
  - 更低回撤
  - 更低波动
  - 略优风险收益比
- 但它没有把合并组合的原始收益进一步做高
- 同时也没有在 `12M` 长窗口下跑赢市场匹配 benchmark 组合

#### 5G.4 当前 12M 基线的正式结论

当前 `12M` 基线允许我们把用户终极问题回答得更完整：

1. `Trade Management v1` 已经有正式的 `12M` 组合级业绩基线，不再停留在短窗口或中窗口。
2. 这套模型当前最稳定的能力是：
   - 降低回撤
   - 降低波动
   - 在部分市场上改善风险收益比
3. 它当前最不应被夸大的能力是：
   - 长窗口下稳定提高原始收益
   - 长窗口下稳定跑赢市场 benchmark
4. 因此，当前用户侧最准确的表述，不是“这是一套已经成熟的跑赢指数模型”，而是：
   - **这是一套已经证明能改善持仓风险结构、并在部分市场环境里提升收益质量的交易管理模型**

下一阶段如果要进一步证明“用户终极问题”，研究重点不应再只是扩窗口，而要进入：
