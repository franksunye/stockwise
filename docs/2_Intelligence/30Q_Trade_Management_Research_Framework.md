...
为了避免后续继续混淆“交易管理与信号发现解耦”这句话，这里把两种评估口径正式写死。

#### A. 系统内正式评估（当前默认）

这是当前 StockWise 已落地的正式研究口径。
...
**在 StockWise 的正式信号与研究 universe 中，这套交易管理是否能改善仓位处理结果。**

说明：

- 这里依赖信号层，不代表交易管理在逻辑上没有解耦
- 它只是说明当前研究对象仍然建立在系统内正式输入链上

#### B. 交易管理独立评估（后续必须补）

这是行业里更纯粹的“交易管理模型”研究口径，也是后续必须补齐的一层。
...
当前阶段，StockWise 已正式完成 A；后续应逐步补齐 B。
...
为了避免闭门造车，交易管理后续所有“更好 / 更差”的判断，不应只建立在：
...
1. `buy-and-hold baseline`
   - 最基础的同入场持有基线
2. `LEAN 因子策略基线`
   - 用成熟公开因子研究框架提供一个可复现的中等复杂度对照
3. `Qlib Alpha158/Alpha360`
   - 作为中国市场常见量化研究基线
4. `Qlib LightGBM`
   - 作为更强但仍属公开范式的模型化基线

这四层的作用不是“替代 StockWise 路线”，而是：
