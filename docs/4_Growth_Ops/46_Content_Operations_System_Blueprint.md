...
其中，针对 `101`、战役长文、公众号文章，审核时新增一个固定问题：

- 这篇稿子是否真的只在讲 `1` 个最想打透的观点？
- 如果是 `TOFU` 公众号稿，正文是否控制在 `800-1200` 字？

判断原则：

- 若超过 `1200` 字，默认先检查是否存在重复论证、补充性解释过多、收尾过慢的问题
- 若一篇稿子里同时在讲多个主命题，默认不能直接过审
- 对 `TOFU` 稿件尤其要检查：前两屏是否已经完成识别、冲突和说服
- 对 `TOFU` 稿件尤其要检查：是否只保留了一个主误区、一个主对比轴、少量关键推进段
- 若后半段仍在重复前半段已完成的说服，默认视为结构冗余，不能直接过审

审核目标不是把文章审成“更完整的说明书”，而是审成“更容易被普通读者读完并记住的传播稿”
