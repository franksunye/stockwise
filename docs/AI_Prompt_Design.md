...
| **分红与停复牌**   | 接口 `stock_fhps_detail_em`          | 自动识别“非交易因素”导致的价格缺口，预警除权除息。 | **P1** | ✅ 已验证 (稳定) |
| **即时消息面感知** | 接口 `stock_news_em`                 | 识别突发利好/利空。                                | **P1** | ⚠️ 不稳定 (防爬) |
| **重大公告提醒**   | 接口 `stock_notice_report`           | 捕捉影响股价中长期走势的确定性事件。               | **P1** | ⚠️ 无个股接口    |
| **北水/机构持仓**  | 接口 `stock_hsgt_stock_holding_em`   | 识别股票背后的底气。                               | **P2** | 🔄 需调研参数    |
| **主力/游资动向**  | 接口 `stock_lhb_detail_em`           | 捕捉大资金进出痕迹。                               | **P2** | 🔄 需调研        |
| **板块背景共振**   | 接口 `stock_board_industry_index_em` | 判断个股是“独秀”还是“陪跑”。                       | **P3** | ⏳ 待办          |
