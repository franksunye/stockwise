...
| 维度               | 实现策略 (基于 AkShare)                                              | 优化目的                                                                    | 优先级 |
| :----------------- | :------------------------------------------------------------------- | :-------------------------------------------------------------------------- | :----- |
| **公司基本面概况** | 接口 `stock_profile_cninfo`：获取**主营业务、经营范围、行业属性**    | 建立 AI 对个股的“第一印象”，理解其赚钱逻辑（如：光伏、房地产、AI 芯片）。   | **P0** |
| **即时消息面感知** | 接口 `stock_news_em`：提取个股近 24h 的前 5 条**新闻标题**           | 识别突发利好/利空（如：被立案调查、签订特大合同），防止 AI 在技术面失灵。   | **P0** |
| **重大公告提醒**   | 接口 `stock_notice_report`：筛选关键词（**减持、并购、财报、预告**） | 捕捉影响股价中长期走势的确定性事件，自动规避公告杀。                        | **P1** |
| **分红与停复牌**   | 接口 `news_trade_notify_suspend_baidu` & `dividend_baidu`            | 自动识别“非交易因素”导致的价格缺口，预警除权除息及停牌风险。                | **P1** |
| **主力/游资动向**  | 接口 `stock_lhb_detail_em` (龙虎榜) & `stock_dzjy_em` (大宗交易)     | 捕捉大资金（如“拉萨团”、“中信证券”）的进出痕迹，为决策增加“跟随动力”。      | **P2** |
| **北水/机构持仓**  | 接口 `stock_hsgt_list_em` (北向资金) & `stock_institute_hold_em`     | 识别股票背后的底气。机构持仓比例增加常被视为中长期走势的“压舱石”。          | **P2** |
| **板块背景共振**   | 接口 `stock_board_industry_index_em` (行业指数行情)                  | 判断个股是“独秀”还是“陪跑”，结合板块强弱指数（Relative Strength）输出建议。 | **P3** |
