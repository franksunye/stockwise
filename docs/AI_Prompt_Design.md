...
| #    | 维度                | 实现方式                                          | 效果                                         |
| :--- | :------------------ | :------------------------------------------------ | :------------------------------------------- |
| 1    | **近5日行情走势**   | 表格 + 趋势信号摘要（连涨/连跌天数 + 累计涨跌幅） | LLM 能感知短期趋势，而非仅看单日             |
| 2    | **月度大周期背景**  | 20月均线 + 月线指标 + 趋势结论                    | LLM 能识别中长期趋势，区分“强反弹”与“真反转” |
| 3    | **板块联动**        | 核心原则第5条，激活 LLM 预训练知识                | 无需额外数据，LLM 自动调用对行业/板块的理解  |
| 4    | **事件驱动**        | 核心原则第6条，尝试激活 LLM 搜索能力              | 如 LLM 有搜索能力会自动使用，否则优雅降级    |
| 5    | **AI 历史预测回顾** | 近5次预测记录 + 全局准确率统计                    | LLM 知道"自己之前说了什么，对不对"，形成闭环 |
...
├── 股票信息（名称、代码、日期）
├── 近5日行情走势（表格 + 趋势信号）
├── 月度大周期背景（月线 MA/RSI 指标、长线趋势结论）
├── 今日行情数据（开高低收、涨跌幅、成交量）
├── 技术指标（MA5/10/20/60、RSI/MACD/KDJ/布林带）
├── AI 历史预测回顾
│   ├── 近5次预测记录（日期、信号、结果、实际涨跌）
│   └── 全局准确率统计（总预测数、正确数、错误数、准确率）
└── 请求
```
