...
| 2    | **板块联动**        | 核心原则第5条，激活 LLM 预训练知识                | 无需额外数据，LLM 自动调用对行业/板块的理解  |
| 3    | **事件驱动**        | 核心原则第6条，尝试激活 LLM 搜索能力              | 如 LLM 有搜索能力会自动使用，否则优雅降级    |
| 4    | **AI 历史预测回顾** | 近5次预测记录 + 全局准确率统计                    | LLM 知道"自己之前说了什么，对不对"，形成闭环 |
| 5    | **Few-shot 示例**   | 完整的 JSON 输出示例                              | 大幅降低格式错误率                           |
