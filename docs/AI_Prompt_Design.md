...
| 维度                   | 实现策略 (基于 AkShare)                                      | 优化目的                                                            | 复杂度 |
| :--------------------- | :----------------------------------------------------------- | :------------------------------------------------------------------ | :----- |
| **个性化持仓上下文**   | 关联 `user_stocks` 表，传入 `cost_price` 与 `position_ratio` | 让建议具备“盈利保护”或“亏损止损”的心理边界，而非单纯分析逻辑。      | 低     |
| **即时消息面感知**     | 接口 `stock_news_em`：提取个股近 24h 的前 5 条新闻标题       | 识别突发利好/利空（如重组、减持），防止 AI 在技术面失效时产生幻觉。 | 中     |
| **板块/大盘共振层**    | 接口 `stock_hk_spot_em` (恒指) + 板块行情                    | 识别“逆市抗跌”或“跟随阴跌”，通过相对强弱 (RS) 判断个股的海拔。      | 中     |
| **公司行为日历**       | 接口 `news_trade_notify_dividend_baidu` (分红/停复牌)        | 自动规避除权除息带来的价格“缺口”误判，以及停牌风险预警。            | 中     |
| **北水动向 (港股通)**  | 接口 `stock_hk_gh_spot_em`：获取港股通持股比例变动           | 捕捉机构/内资大资金的吸筹轨迹，为“置信度”增加权重。                 | 高     |
| **LLM 搜索增强 (RAG)** | 启用 Gemini Google Search Grounding 或自定义新闻向量库       | 解决 LLM 知识库的滞后性，获取分析师研报中未包含的最新市场流言。     | 高     |
