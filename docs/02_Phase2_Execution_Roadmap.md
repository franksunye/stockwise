...
*   **10k 标准**: 
    ...
    *   **新闻挂了** -> 降级为纯技术分析模式。
    *   **AI 挂了** -> 降级为规则引擎模式。
*   **对策**: 完善 `StrategyFactory` 的 Fallback机制。
...
- [x] **去重与缓存机制**: 确保同一股票在同一天只被 AI 分析一次。结果存入 `ai_predictions` 表，其余用户直接读取 (Read-Model)。(这将极大降低 API 成本和时间)。
- [x] **错误隔离**: 单个股票/用户的失败绝不能阻塞整个 Batch 任务。
- [x] **EastMoney API 增强**: 增加重试机制和 Result Parsing 的健壮性。

### P1: 智能信号逻辑 (Intelligence)
- [x] **实现 Signal Flip 检测**: 在写入新预测前，读取旧预测，计算 `Status` (Upgraded/Downgraded/Unchanged)。
- [x] **通知分级 (Notification Tiering)**: 
    - 紧急 (Flip): 立即推送 / 单独推送。
    - 普通 (Daily): 合并在“日报”中推送。

### P2: 用户体验打磨 (Delight)
- [x] **简报排版优化**: 确保 AI 输出的 Markdown 在手机端阅读体验极佳（关键数字高亮）。
- [x] **推送文案优化**: 告别机械的 "日报已生成"，转向 "腾讯控股出现买入信号..." 这样高点击率的文案。
