...
- 明确哪些接口负责哪一层数据；
- 明确哪些接口**不得**用于高频轮询，避免 `ai_reasoning` 这类日级重 payload 被误用在 约 5 分钟刷新场景。
...
- **Layer B — 盘中价量快照层 (Intraday Price Snapshot)**  
  ...
  - **更新节奏**：交易时段内大约每 约 5 分钟刷新一次（但仍属于快照，而非逐笔行情）。
...
- `GET /api/stock/batch`
  ...
  - 更新节奏：按交易日（或显式刷新）使用，不作为 约 5 分钟轮询接口。
  - 当前国际版 v1 语义：
...
如果前端把 `/api/stock/batch` 当成「价格刷新接口」每 约 5 分钟调用一次，将导致：
...
1. **禁止使用 `/api/stock/batch` 作为价格轮询接口。**
   - 任何「每 约 5 分钟刷新一次价格」的需求，都必须走专门的价格视图 API（未来的 `/api/stock/prices` 系列）。
...
- 当前系统自然分为两层：
  ...
  - **盘中价量快照层 (Layer B)**：价格、短期指标，每 约 5 分钟更新一次。
