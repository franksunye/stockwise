...
更新时间：2026-03-13  
状态：Proposal  
定位：将量化模型结论与分析模型原始结论/分析过程同时资产化的双轨决策方案
...
1. 量化模型（当前工程实现为 `Layer-1 / QuantEngine`）先给出结构化结论
2. 分析模型（当前主要是 LLM）在量化模型约束下生成解释、战术与报告
3. 若 LLM 原始 `signal` 与量化模型不一致，由主链 enforcement 收口
...
本方案的目标不是削弱量化模型，也不是把 LLM 放回“随意决定方向”的位置。
...
为避免未来扩展时继续混淆，当前统一按以下链路理解：

1. 量化方法框架
   - 例如趋势跟随、突破、均值回归、网格、价值、配置
2. 量化规则组件
   - 例如 Donchian、ATR、MA filter、volume expansion
3. 量化模型
   - 当前工程实现主要对应 `Layer-1 / QuantEngine`
   - 输出 `layer1_status`
4. 分析模型
   - 当前主要对应 LLM
   - 输出 AI 原始判断与解释
5. 投资模式
   - steady / balanced / aggressive / observe-only
   - 对同一底层事实做不同风格裁决
6. 视图层
   - 决定首页、策略内参、投资决议页各自展示哪条轨

当前双轨方案主要处理的是第 3 层与第 4 层的解耦，不直接改变第 5 层与第 6 层的产品策略。
...
**当前最值得做的，不是继续把分析模型完全压进量化模型，而是把“量化纪律结论”和“AI 原始判断/解释”同时资产化，让后台分析价值和前端展示价值都最大化。**
