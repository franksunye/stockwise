...
class FixedDisciplineExitPolicy(ManagementPolicy):
    policy_id = "discipline_exit_only"
    ...
    def decide(self, position_state: PositionState, context: PolicyContext) -> list[ManagementAction]:
        if position_state.failed_breakout_risk and position_state.discipline_price:
            return [
                ManagementAction(
                    action="EXIT_ALL",
                    trigger_reason=f"有效跌破纪律位 {position_state.discipline_price:.2f}",
                    stop_price=position_state.discipline_price,
                )
            ]
        return [ManagementAction(action="HOLD", trigger_reason="纪律未破，继续持有")]
