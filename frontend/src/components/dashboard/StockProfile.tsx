...
export function StockProfile({ stock, isOpen, onClose }: StockProfileProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-[200] bg-[#050508] flex flex-col pointer-events-auto touch-auto animate-in slide-in-from-bottom duration-300 ease-out"
    >
      {/* 顶部视觉拉手 (仅视觉装饰) */}
      <div 
        className="w-full flex justify-center pt-3 pb-1"
        onClick={onClose} // 点击顶部也可以关闭
      >
      ...
      <div className="h-full w-full p-8 pt-6 flex flex-col overflow-y-auto">
        ...
        <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 px-2">
          复盘矩阵 (最近 30 天) {loadingHistory && <span className="text-indigo-500 opacity-70">加载中...</span>}
        </h3>
